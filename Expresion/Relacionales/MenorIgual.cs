...
{
    public class MenorIgual : Nodo
    {
        private Nodo left;
        private Nodo right;
        public MenorIgual(int linea, int columna, Nodo left, Nodo right) : base(linea, columna)
        {
            this.left = left;
            this.right = right;
        }
        public override Retorno compilar(Entorno entorno)
        {
            ...
            switch (res_left.getTipo())
            {
                ...
                case Objeto.TipoObjeto.REAL:
                    ...
                    switch (res_right.getTipo())
                    {
                        ...
                        default:
                            ...
                            "No se puede aplicar menor igual que a tipos de datos " + res_left.getTipo().ToString() + " con " + res_right.getTipo().ToString());
                            Master.getInstancia.addError(error);
                            throw new Exception("No se puede aplicar menor igual que a tipos de datos " + res_left.getTipo().ToString() + " con " + res_right.getTipo().ToString());
                    }
                ...
                default:
                    ...
                    "No se puede aplicar menor igual que a tipos de datos " + res_left.getTipo().ToString());
                    Master.getInstancia.addError(error3);
                    throw new Exception("No se puede aplicar mayor igual que a tipos de datos " + res_left.getTipo().ToString());
            }
        }
        public Retorno Llamada_Nativa_Comparar(Retorno res_left, Retorno res_right, int size)
        {
            ...
            Master.getInstancia.callFuncion("native_menor_igual_str");
